//
//  HBIdentifyCodeViewController.m
//  HeartBeat1.0
//
...
@interface HBIdentifyCodeViewController ()

@end

@implementation HBIdentifyCodeViewController
...
-(void)clickedNextButton:(UIButton* )sender {
    if (self.identifyCodeTF.text.length == 0) {
        ...
        if ([self.identifyCodeTF.text isEqualToString:self.verifyCode]) {
            HBAddUserInfoViewController* HBAddUserInfoVC = [[HBAddUserInfoViewController alloc]init];
            [self.navigationController pushViewController:HBAddUserInfoVC animated:NO];
        }else {
